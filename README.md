...
To use esp run the following command on Centos 5 or 6:
    ...
    # source /dev/stdin <<< "$(curl -sL https://raw.githubusercontent.com/sam2332/ESP/master/esp)"
...
Example:
    ...
    # ssp=1 source /dev/stdin <<< "$(curl -sL https://raw.githubusercontent.com/sam2332/ESP/master/esp)"
...
Now you can run esp and esp will detect the hook and load ~/.esprc if it exists.  Addtionaly you can combine the function, with configuration options and the esp command for an (admitdly large) one liner. 
    ...
    esp_check_disable_mysql=1 function esp_hook_pre { [ -f ~/.esprc ] && source ~/.esprc;}; source /dev/stdin <<< "$(curl -sL https://raw.githubusercontent.com/sam2332/ESP/master/esp)"
...
Like the pre hook you can place the function before the esp command for an one liner:
    ...
    function esp_hook_post { export PS1="$PS1 >>>"; }; source /dev/stdin <<< "$(curl -sL https://raw.githubusercontent.com/sam2332/ESP/master/esp)"
